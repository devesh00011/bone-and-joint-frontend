                    ...
                    <div className='grid lg:grid-cols-[30%_auto] grid-cols-1 gap-10'>
                        <div>
                            ...
                            <div className='p-5 rounded-2xl mb-6 bg-white h-fit shadow-xl border-2 border-gray-300'>
                                <h2 className='relative text-2xl font-semibold text-gray-800 mb-5'>Categories
                                    <div className='absolute top-full left-0 w-12 h-1 rounded-full bg-blue-700'></div>
                                </h2>
                                <button onClick={() => setSelectedCategory('all')} className={`${selectedCategory == 'all' ? 'bg-blue-600 text-white' : 'bg-white text-bg-blue-600'} hover:bg-blue-600 hover:text-white  rounded-full cursor-pointer border-2 border-gray-400 hover:scale-[1.1] duration-100 px-4 py-1 hover:border-transparent border-bg-blue-600 m-2`}>All</button>
                                {data.map((item, index) => {
                                    return (
                                        <button onClick={() => setSelectedCategory(item)} key={index} className={`${selectedCategory == item ? 'bg-blue-600 text-white' : 'bg-white text-bg-blue-600'} rounded-full cursor-pointer hover:bg-blue-600 hover:text-white duration-100 border-2 border-gray-400 px-4 py-1 shadow-md hover:scale-[1.1] hover:border-transparent border-bg-blue-600 m-2.5`}>{item}</button>
                                    )
                                })}
                        ...
                        <div className='grid sm:grid-cols-2 gap-10'>
                            {[1, 2, 3, 4, 5, 6].map((item, index) => {
                                return (
                                    <Link key={index} href={`/blogs/category/${item}`}><div key={index} className='group border-2 cursor-pointer bg-white border-gray-200 hover:shadow-md rounded-2xl'>
                                        ...
                                        <div className='p-5'>
                                            <h2 className='text-xl font-semibold group-hover:text-blue-600 duration-300'>{index + 1}. What is difference between div and section ?</h2>
                                            <p className='border-b-2 border-gray-200 pb-1.5 line-clamp-2 group-hover:text-blue-600 duration-300'>Lorem ipsum dolor sit amet consectetur adipisicing elit. Nobis dicta soluta placeat minus, delectus sint veritatis, ipsum id incidunt aperiam.</p>
                                            <p></p>
                                            <p className='mt-1.5 text-gray-800 font-semibold'>Dr. Rajiv Chouhan</p>
